            ...
            <Formik
              ...
              onSubmit={async (values) => {
                try {
                  ...
                  // Check if it's a 400 error (validation/duplicate data)
                  ...
                  if (status === 400) {
                    errorMessage =
                      "Não foi possível completar o cadastro. Verifique se os dados informados estão corretos e tente novamente.";
                  } else if (status === 500) {
                    errorMessage =
                      "Erro no servidor. Por favor, tente novamente mais tarde.";
                  } else if (status === 409) {
                    errorMessage =
                      "Já existe uma conta com esses dados. Tente fazer login ou use dados diferentes.";
                  } else if (!error.response) {
                    errorMessage =
                      "Erro de conexão. Verifique sua internet e tente novamente.";
                  }

                  Snackbar.show({ text: errorMessage, type: "error" });
                }
              }}
            >
              {({
                ...
                const validateStep = async () => {
                  ...
                  const stepErr = current.reduce((acc, key) => {
                    if ((allErrors as any)[key])
                      acc[key] = (allErrors as any)[key];
                    return acc;
                  }, {} as Record<string, string>);
                  if (Object.keys(stepErr).length) {
                    Snackbar.show({
                      text: Object.values(stepErr)[0],
                      type: "error",
                    });
                    return false;
                  }
                ...
                const handleCepChange = async (text: string) => {
                  ...
                  if (cleaned.length === 8) {
                    ...
                    try {
                      const { uf, city, address } = await fetchAddressByCep(
                        cleaned
                      );
                      setFieldValue("uf", uf);
                  ...
                  <View>
                    ...
                    {step === 2 && (
                      <>
                        ...
                        <TextInput
                          ...
                          value={values.phone}
                          onChangeText={(t) =>
                            setFieldValue("phone", maskPhone(t))
                          }
                      ...
                      <>
                        <View style={styles.toggleContainer}>
                          ...
                          <TouchableOpacity
                            ...
                            onPress={() =>
                              setFieldValue("role", "professional")
                            }
                          >
                    ...
                    <View style={styles.buttonRow}>
                      ...
                      <TouchableOpacity
                        ...
                        onPress={
                          step < TOTAL_STEPS ? nextStep : (handleSubmit as any)
                        }
                        disabled={isSubmitting || loading}
                      >
